...
categor = ['category']
data_cate = data.select_dtypes(include=categor)

```

```{python}
##### Data Prepare for Modeling
data=data.drop(['Customer_ID','Type_of_Loan'], axis=1)
data = pd.get_dummies(data, columns = ['Month','Occupation','Credit_Mix','Payment_of_Min_Amount']) # create dummy variable
le = LabelEncoder() # create labeling
data['Credit_Score']=le.fit_transform(data['Credit_Score']) # labeling target variable
data=data.drop(['Credit_Mix_0','Occupation_0'], axis=1) # Remove column

data_test,data_train = train_test_split(data, test_size=0.7) # Data split by 0.7
data_train.Credit_Score.value_counts() # Group by count Target variable

def evaluate(model, test_features, test_labels): # function to calculate accuracy
